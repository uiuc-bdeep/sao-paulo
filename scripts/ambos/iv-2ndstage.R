...
# (for subset of trips where both directions of traffic are affected)                    #
#                                                                                        #
#                                                                                        #
...
coef.path <- "intermediate/floods/iv2-coef(ambos).rds"
out <- "views/floods/"
...
# the variable 'ambos' gives us the number of floods a trip encounters where both directions of traffic are affected 
...
iv.1 <- felm(tr.time ~ fitted.blocks + fitted.floods + fitted.spill | ID_ORDEM | 0 | ID_ORDEM, data = trips)
...
iv.2 <- felm(tr.time ~ fitted.blocks + fitted.floods + fitted.spill | ID_ORDEM + month | 0 | ID_ORDEM, data = trips)
...
iv.3 <- felm(tr.time ~ fitted.blocks + fitted.floods + fitted.spill | ID_ORDEM + month + wd | 0 | ID_ORDEM, data = trips)
...
iv.4 <- felm(tr.time ~ fitted.blocks + fitted.floods + fitted.spill | ID_ORDEM + month + wd + hour.f | 0 | ID_ORDEM, data = trips)
...
stargazer(iv.1, iv.2, iv.3, iv.4, 
          ...
          out = paste0(out.path, "iv-secondstage(ambos).tex"))
